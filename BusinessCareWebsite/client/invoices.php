...
<main class="form-section">
    ...
    <?php if (empty($invoices)): ?>
        <div style="padding: 20px; background-color: #fff3cd; border-left: 5px solid #ffc107; border-radius: 6px;">
            Aucune facture disponible pour votre entreprise.
        </div>
    <?php else: ?>
            ...
            <tbody>
                <?php foreach ($invoices as $facture): ?>
                    <tr style="border-bottom: 1px solid #ccc; background-color: #f9f9f9;">
                        ...
                        <td style="padding: 12px;">
                            <?php if ($facture['status'] === 'paid'): ?>
                                <a href="../provider/download_invoice.php?id=<?= $facture['id'] ?>" target="_blank" style="color: #007bff; font-weight: bold;">Télécharger le PDF</a>
                            <?php else: ?>
                                <a href="payer.php" style="color: #dc3545; font-weight: bold;">Payer maintenant</a>
                            <?php endif; ?>
                        </td>
                    </tr>
                <?php endforeach; ?>
